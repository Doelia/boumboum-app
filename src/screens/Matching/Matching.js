...
export default function Matching({}) {
    ...
    useEffect(() => {
        ...
        let cb = onNextSubscriber.current.subscribe((isYes) => {
            if (!isYes) return;
    ...
    return (
        <>
            ...
            <View style={{flex: 1, justifyContent: 'space-between'}}>
                ...
                <BlurredBackground stackProfiles={stackProfiles} currentIdBackground={currentIdBackground}
                ...
                <View style={{flex: 1, paddingHorizontal: 20, paddingVertical: 20, ...CENTER}}>
                    <View style={{...P100, ...CENTER, maxWidth: 400, maxHeight: 600, alignItems: 'center'}}>
                        {stackProfiles.map((profile, index) => (
                            <Card setCurrentIdBackground={setCurrentIdBackground} index={index} key={profile.id}
                                  profile={profile} onNext={onNextSubscriber}></Card>
                        ))}
                    </View>
                </View>
