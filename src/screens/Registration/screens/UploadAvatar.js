        ...
        <View style={{flex: 1}}>
            ...
            <View style={{flex: 1, ...CENTER}}>
                <TouchableOpacity onPress={pick} style={styles.circle}>
                    <View style={{ flex: 1, backgroundColor: '#ececec', borderRadius: circleSize, margin: 8, overflow: 'hidden', ...CENTER }}>
                        {image && <Image source={{uri: image}} style={{ ...P100, objectFit: 'cover'}}/>}
                        {!image && <Image source={IMAGES.registration.icon_user} style={{width: '40%', height: '40%', objectFit: 'cover'}}/>}
                    </View>
...
const styles = {
    circle: {
        height: circleSize, width: circleSize, backgroundColor: 'white', borderRadius: 200, borderWidth: 2, borderStyle: 'dotted', borderColor: COLORS.PINK
    },
    image_btn_edit: {
        width: 122 / 2,
        height: 122 / 2,
